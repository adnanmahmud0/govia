...
import 'package:gsabino365/core/utils/logger.dart';
...
class ApiClient extends GetxService {
  ...
  // Prevents double-logout race: set to true the moment any logout begins.
  static bool _isLoggingOut = false;

  /// Called by AuthService at the start of a user-initiated logout so that
  /// any concurrent 401s from still-alive controllers are silently ignored.
  static void markLoggingOut() => _isLoggingOut = true;
  static void clearLoggingOut() => _isLoggingOut = false;
  ...
  /// Force logout when refresh fails — only fires once per session.
  void _forceLogout() {
    if (_isLoggingOut) return; // Guard: already logging out
    _isLoggingOut = true;
    StorageService.clearAll();
    Get.offAllNamed(AppRoutes.login);
    Helpers.showError('Please login again.', title: 'Session Expired');
    // Reset flag after navigation is committed so future logins work
