...
class DoctorLiveCallController extends GetxController with WidgetsBindingObserver {
  final MeetingRepository meetingRepo;

  DoctorLiveCallController({required this.meetingRepo});
  ...
  String? meetingId;
  String roomName = 'govia_crisis_consultation';
  ...
  void onInit() {
    ...
    CallBackgroundService.start(
      title: 'GoVia Medical Consultation',
      text: 'Call active • Live audio & video',
    );
    _resolveDoctorLicense();
    _extractMeetingArguments();
    joinCallSession();
  }

  void _resolveDoctorLicense() {
    if (Get.isRegistered<AuthService>()) {
      final user = Get.find<AuthService>().currentUser.value;
      if (user != null) {
        final lic = user.medicalLicenseNumber ??
            user.assignedNumber ??
            user.badgeNumber ??
            user.shortHexId;
        if (lic != null && lic.isNotEmpty) {
          licenseText.value = 'License: #$lic';
        }
      }
    }
  }
  ...
  void _extractMeetingArguments() {
    ...
    if (args is MeetingModel) {
      ...
      roomName = m['roomName']?.toString() ??
          ...
          (meetingId != null && meetingId!.isNotEmpty ? 'govia_$meetingId' : 'govia_crisis_consultation');
  ...
  Future<void> joinCallSession() async {
    ...
    if (token == null || token.isEmpty) {
      ...
      if (Get.isRegistered<AuthService>()) {
        ...
        doctorName = auth.currentUser.value?.name ?? doctorName;
        doctorId = auth.currentUser.value?.id ?? doctorId;
      }
    ...
    try {
      ...
      debugPrint('⚠️ Error joining doctor call: $e');
      isLoading.value = false;
      hasError.value = true;
      errorMessage.value = 'Failed to connect to medical video session: $e';
    }
  }
  ...
  void _setupLiveKitListeners(EventsListener<RoomEvent> listener, Room room) {
    listener
      ..on<RoomDisconnectedEvent>((event) {
        debugPrint('🔴 Doctor LiveKit RoomDisconnected');
      ...
      ..on<DataReceivedEvent>((event) {
        try {
          ...
          if (msg == 'meeting_ended') {
            debugPrint('🔔 Host ended meeting — concluding doctor session');
            _onHostEndedMeeting();
  ...
  void _updateTracks(Room room) {
    ...
    // Remote video track (Patient / Encounter camera stream)
      ...
      for (final pub in participant.videoTrackPublications) {
        if (pub.track is VideoTrack) {
          ...
          caller = participant.name.isNotEmpty
              ? participant.name
              : (participant.identity.isNotEmpty ? participant.identity : 'Patient');
          break;
        }
      }
  ...
  Future<void> toggleSpeaker() async {
    ...
    try {
      ...
      debugPrint('Error toggling doctor speakerphone: $e');
    }
  }
  ...
  Future<void> leaveCall() async {
    ...
    await _cleanupRoom();
    Get.back();
    Helpers.showSuccess('Medical consultation session concluded.');
  }
  ...
  Future<String?> getMeetingJoinQrPayload() async {
    ...
    return jsonEncode({
      ...
      'meetingId': mId,
      'roomName': roomName,
      'topic': meeting?.topic ?? topic.value,
      'token': guestToken ?? '',
      'livekitUrl': meeting?.livekitUrl ?? '',
      'category': meeting?.category ?? 'CONSULTATION',
      'meetingType': meeting?.meetingType ?? 'EMERGENCY',
    });
  }
  ...
  Future<void> joinFromQrPayload(String jsonPayload) async {
    try {
      ...
      final url = map['livekitUrl']?.toString() ??
          ...
          'wss://govia-0f13ke90.livekit.cloud';
  ...
  void _onHostEndedMeeting() {
    ...
    _cleanupRoom();
    Get.back();
    Helpers.showError('The session has ended.', title: 'Session Concluded');
  }
  ...
  Future<void> _cleanupRoom() async {
    ...
    try {
      ...
      debugPrint('Error cleaning up doctor room: $e');
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    debugPrint('📱 [DoctorLiveCallController] App lifecycle changed: $state');
