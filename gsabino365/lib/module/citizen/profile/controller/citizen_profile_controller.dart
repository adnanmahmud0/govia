...
class CitizenProfileController extends GetxController {
  ...
  final RxString preferredAttorney = ''.obs;
  final RxString preferredBailBondsman = ''.obs;
  ...
  void onInit() {
    ...
    // Listen for changes to auth currentUser
    ever<UserModel?>(_authService.currentUser, _syncUserData);

    // Initial sync with cached user
  ...
  void _syncUserData(UserModel? user) {
    ...
    name.value = user.name?.isNotEmpty == true ? user.name! : 'Citizen';
    ...
    if (user.preferredAttorney != null && user.preferredAttorney!.isNotEmpty) {
      preferredAttorney.value = user.preferredAttorney!;
    }
    if (user.preferredBailBondsman != null &&
        user.preferredBailBondsman!.isNotEmpty) {
      preferredBailBondsman.value = user.preferredBailBondsman!;
    }
  ...
  void openQrDialog({int initialTabIndex = 0}) {
    UserQrCardDialog.show(
      userName: name.value.isNotEmpty ? name.value : 'Citizen',
      userRole: activeRole.value.isNotEmpty ? activeRole.value : 'Citizen',
