...
class MentalHealthRegisterController extends GetxController {
  ...
  final rolesList = ['Psychiatrist', 'Psychologist', 'Counselor', 'Therapist'];
  ...
  Future<void> register() async {
    ...
    final payload = <String, dynamic>{
      ...
      'subRole': rxSelectedRole.value ?? 'Therapist',
    ...
    if (licenseNumberController.text.trim().isNotEmpty) {
      payload['medicalLicenseNumber'] = licenseNumberController.text.trim();
    }
    if (specializationController.text.trim().isNotEmpty) {
      payload['specialization'] = specializationController.text.trim();
    }
    ...
    try {
      ...
      if (statusCode == 200 || statusCode == 201) {
        Helpers.showSuccess(
          'Mental Health Professional account created! Please verify OTP.',
